...
---------------------------------------
- Descarga 30 días de tipos de cambio desde la API de Frankfurter (datos BCE, gratis, sin key).
...
API_URL = "https://api.frankfurter.app/{start}..{end}?from=EUR&to=CLP,USD"


# ----------------------------- Datos -----------------------------------------

def fetch_history(dias: int) -> dict:
    """Descarga el histórico EUR→{CLP,USD}. Pide un margen extra por fines de semana."""
    end = date.today()
    ...
    try:
        ...
        raise SystemExit(f"No se pudo conectar a Frankfurter ({url}): {e}")
...
<header>
  ...
  <div class="sub">Datos: api.frankfurter.app (BCE) · Generado: {datetime.now():%Y-%m-%d %H:%M}</div>
</header>
<main>
...
<footer>Frankfurter publica solo días hábiles del BCE (sin fines de semana ni festivos).</footer>
<script>{js}</script>
</body></html>
"""
...
def main() -> None:
    ...
    print(f"→ Descargando histórico de {args.dias} días desde Frankfurter…")
